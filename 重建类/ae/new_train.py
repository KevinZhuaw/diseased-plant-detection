...
class Config:
    ...
    LATENT_DIM = 256  # 减少潜在空间维度，节省显存
    CHANNELS = 3  # RGB图像

    # 训练配置
    BATCH_SIZE = 16  # 减少批处理大小，4070ti可以处理
...
class LightweightAutoencoder(nn.Module):
    """轻量级自编码器+分类器模型，减少显存使用"""

    def __init__(self, input_channels=3, latent_dim=256, num_classes=22):
        super(LightweightAutoencoder, self).__init__()

        # 编码器 - 减少通道数
        self.encoder = nn.Sequential(
            # 输入: (3, 224, 224)
            nn.Conv2d(input_channels, 32, kernel_size=4, stride=2, padding=1),  # (32, 112, 112)
            nn.BatchNorm2d(32),
            nn.ReLU(True),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(32, input_channels, kernel_size=4, stride=2, padding=1),  # (3, 224, 224)
            nn.Sigmoid()
        )
...
def validate_model_dimensions():
    ...
    # 创建测试模型
    test_model = LightweightAutoencoder(input_channels=3, latent_dim=Config.LATENT_DIM, num_classes=22)
...
def main():
    ...
    transform = transforms.Compose([
        ...
        transforms.RandomHorizontalFlip(p=0.2),  # 减少数据增强概率
        transforms.ToTensor(),
    ])
